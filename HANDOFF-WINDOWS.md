...
**交互说明（已按多轮反馈重做）：** 窗口**尺寸自适应内容**（收起=小药丸/展开=面板，不再是大灰框）。纯透明（无 backdrop-filter，去 acrylic）—— 桌面/后面窗口能透出。
- 收起态：**整条药丸任意位置可拖动**；点 `⌄` 展开详情。
- 展开态：**标题栏可拖动**；点 `✕` 收起。
- 收起↔展开只显示一个，带 scale+fade 切换动效。
- 卡住提示：running 且 ≥10min 无 hook 回调 → 计时变黄，悬停显示「已 X 分钟无 Claude 回调」。
