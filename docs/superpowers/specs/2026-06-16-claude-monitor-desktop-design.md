...
- 日期：2026-06-16
- 状态：已确认；Phase A/B 已实现；**架构修订见下方**
- 关联：复用并增强既有仓库 `claude-monitor`（hook 脚本 + `monitor_server.py`，原为 ESP32 物理指示器所建）

> **修订（2026-06-16，实测后）：** 后端 `monitor_server.py` 改为**原生跑在 Windows**（随桌面项目 `server/`，Tauri 启动时原生 spawn python），不再放 WSL、不再用 `wsl.exe` 拉起。原因：`wsl.exe ... &` 拉起的后台进程会随 wsl 会话结束被回收（实测 server 起不来）。镜像网络下 WSL 的 hook POST 到 `127.0.0.1:8787` 已验证可达，事件端到端落到 Windows server。WSL 侧只需配 hooks。本修订影响 §2 架构与 §5.5。
...
### 5.5 后端连通与拉起（修订：Windows 原生）
- 后端 `server/monitor_server.py` 随桌面项目，原生跑在 Windows、监听 `127.0.0.1:8787`。
- 连接：Tauri 前端 GET `http://localhost:8787/state`。
- 启动自检：探 8787 端口（std `TcpStream`）；未监听则 Tauri **原生 spawn python** 跑 server（同机进程，不随会话回收）。
- WSL 侧：被监控的 Claude Code 经 hook POST 到 `127.0.0.1:8787`（镜像网络 loopback，已验证）。
